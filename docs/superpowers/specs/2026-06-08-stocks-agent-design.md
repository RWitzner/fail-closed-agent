...
- **Date:** 2026-06-08
- **Status:** **Active — M2 market-state implemented & hardened** (532 tests; branch `m2-market-state`; M0 + M1 + M2 done, M1 tier-2 2b live deferred pending the paid live subscription; **M3 next**; each milestone has its own plan/review). Design reviewed twice externally + adversarial workflows/code reviews.
- **Owner:** Robin
...
| **M3 — Signal + calibration probe** | **NEXT** — feature engine; observe-only calibration probe (`paper_eligible=False`). | S1 (probe opens nothing); probe logs forecasts + realized-move scoring; calibration report renders. |
